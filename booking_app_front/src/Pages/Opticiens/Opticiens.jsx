...
import { opticiens } from '../../assets/data';
...
const Opticien = () => {
  const {uri}= useParams()
  const optician=opticiens.find((opt)=>opt.uri==uri)

  const[activeImage,setActiveImage]=useState(optician.images[0]);
  ...
  return (
    <div className="w-9/12 max-sm:w-full mx-auto bg-transparent shadow-lg  overflow-hidden padding  text-night">
        ...
        <div className="flex-1 px-4 max-sm:px-6 max-lg:px-8">
            <h2 className="headline first-letter:text-customBlue  ">{optician.nom}</h2>
            <hr className='w-1/2' />
            <p className="italic mb-4 text-lg font-semibold py-10 w-11/12 font-sansBody">{optician.slogan}</p>
            <div className='flex flex-wrap w-full my-4 max-md:my-2'> {optician.images.map((image)=>(
        <img 
          ...
          style={{backgroundImage: `url(${activeImage})`,height:"80vh"}}>
            ...
            <div className="my-5 text-lg max-md:text-base ">
                ...
                <div className=' padding w-full text-night font-sansBody font-semibold'>
                 <strong className='text-lg font-bold  xl:headline'>Description :</strong> <p className=" px-6  text-left  "> {optician.description}</p>
                ...
                {/* <p className=" p-5 px-10 border-2 border-customBlue mx-4"><strong className='text-night text-lg'>Adresse:</strong> {optician.adresse}</p> */}
                {/* { optician.téléphone ?
                <p className=" p-5 px-10 border-2 border-customBlue mx-4"><strong className='text-night text-lg'>Téléphone:</strong> {optician.téléphone}</p>
                :""} */}
                {optician.prix ? 
                <p className=" p-5 px-10 border-2 border-customBlue mx-4"><strong className='text-night text-lg'>Prix:</strong> {optician.prix}</p> 
                :""
                }
                {optician.horaires ? 
                <p className=" p-5 px-10 border-2 border-customBlue mx-4"><strong className='text-night text-lg'>Horaires:</strong> {optician.horaires}</p>
                :""}
                {optician.informations_importants?
                optician.informations_importants.notes? 
                 <p className='p-5 px-10 border-2 border-customBlue mx-4 '><strong className='text-night text-lg'>Notation  :</strong> {optician.informations_importants.notes}</p>
                :"":""}

                {/* {optician.siteweb ?
                <p className='capitalize p-5 px-10 border-2 border-customBlue font-bold leading-10 mx-4' >pour visiter le site Web de ce optician Cliquer sur :<a href={optician.siteweb} target="_blank" rel="noopener noreferrer" className="font-bold text-white bg-Lion p-2  ">site web <FontAwesomeIcon icon={faLink} /> </a></p>
                :""} */}
                ...
                <div className='bg-white w-1/5 max-md:w-6/12 max- h-3 rounded-xl mx-4 '>
                    ...
                    style={{width:`${averageRating(optician.avis)*100/5}%`}}>
                    </div>
                  </div>
                  <span className="text-lg font-semibold">  { averageRating(optician.avis)} / 5 </span>
                ...
                <div className="mt-6  justify-start flex-wrap">
                  {optician.avis.map((review, index) => (
