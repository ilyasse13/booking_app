...
const Opticien = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden mt-10">
      <img className="w-full h-64 object-cover" src={optician.images[0]} alt={`Image of ${optician.nom}`} />
      <div className="p-6">
        <h2 className="text-2xl font-bold mb-2">{optician.nom}</h2>
        <p className="text-gray-700 mb-4">{optician.description}</p>
        <p className="text-gray-700 mb-2"><strong>Adresse:</strong> {optician.adresse}</p>
        <p className="text-gray-700 mb-2"><strong>Téléphone:</strong> {optician.téléphone}</p>
        <p className="text-gray-700 mb-2"><strong>Horaires:</strong> {optician.horaires}</p>
        <a href={optician.résérvation} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline mb-4 block">Réserver un rendez-vous</a>
        
        <div className="flex items-center">
          <span className="text-lg font-semibold">{averageRating(optician.avis)}</span>
