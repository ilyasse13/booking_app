...
const Restaurant = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden mt-10">
      <img className="w-full h-64 object-cover" src={restaurant.images[0]} alt={`Image of ${restaurant.nom}`} />
      <div className="p-6">
        <h2 className="text-2xl font-bold mb-2">{restaurant.nom}</h2>
        <p className="text-gray-700 italic mb-4">{restaurant.slogan}</p>
        <p className="text-gray-700 mb-4">{restaurant.description}</p>
        <p className="text-gray-700 mb-2"><strong>Adresse:</strong> {restaurant.adresse}</p>
        <p className="text-gray-700 mb-2"><strong>Téléphone:</strong> {restaurant.téléphone}</p>
        <p className="text-gray-700 mb-2"><strong>Prix:</strong> {restaurant.prix}</p>
        <p className="text-gray-700 mb-4"><strong>Horaires:</strong> {restaurant.horaires}</p>
        <a href={restaurant.siteweb} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline mb-4 block">Visiter le site web</a>
        
        <div className="flex items-center">
          <span className="text-lg font-semibold">{averageRating(restaurant.avis)}</span>
