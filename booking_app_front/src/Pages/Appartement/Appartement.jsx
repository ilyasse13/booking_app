...
const Appartement = () => {
  ...
  const { uri } = useParams();
  const hebergements = t('hébergements',{returnObjects:true});
  ...
  return (
    <div className="w-9/12 max-sm:w-full mx-auto bg-transparent shadow-lg  overflow-hidden padding  text-night">
        ...
        <div className="flex-1 px-4 max-sm:px-6 max-lg:px-8">
            <h2 className="headline first-letter:text-customBlue  ">{hebergement.nom}</h2>
            <hr className='w-1/2' />
            <p className="italic mb-4 text-lg font-semibold py-10 w-11/12 font-sansBody">{hebergement.slogan}</p>
            <div className='flex flex-wrap w-full my-4 max-md:my-2'> {hebergement.images.map((image)=>(
        <img 
          ...
          style={{backgroundImage: `url(${activeImage})`,height:"80vh"}}>
            ...
            <div className="my-5 text-lg max-md:text-base ">
                ...
                <div className=' padding w-full text-night font-sansBody font-semibold'>
                 <strong className='text-lg font-bold  xl:headline'>{t('description_title')}:</strong> <p className=" px-6  text-left  "> {hebergement.description}</p>
                ...
                {hebergement.prix ? 
                <p className=" p-5 px-10 border-2 border-customBlue mx-4"><strong className='text-night text-lg'>{t('prix')}:</strong> {hebergement.prix}</p> 
                :""
                }
                {hebergement.horaires ? 
                <p className=" p-5 px-10 border-2 border-customBlue mx-4"><strong className='text-night text-lg'>{t('Horaires')}:</strong> {hebergement.horaires}</p>
                :""}
                {hebergement.informations_importants?
                hebergement.informations_importants.notes? 
                 <p className='p-5 px-10 border-2 border-customBlue mx-4 '><strong className='text-night text-lg'>{t('Notation')}  :</strong> {hebergement.informations_importants.notes}</p>
                :"":""}

                {/* {hebergement.siteweb ?
                <p className='capitalize p-5 px-10 border-2 border-customBlue font-bold leading-10 mx-4' >pour visiter le site Web de ce hebergement Cliquer sur :<a href={hebergement.siteweb} target="_blank" rel="noopener noreferrer" className="font-bold text-white bg-Lion p-2  ">site web <FontAwesomeIcon icon={faLink} /> </a></p>
                :""} */}
                ...
                <div className='bg-white w-1/5 max-md:w-6/12 max- h-3 rounded-xl mx-4 '>
                    ...
                    style={{width:`${averageRating(hebergement.avis)*100/5}%`}}>
                    </div>
                  </div>
                  <span className="text-lg font-semibold">  { averageRating(hebergement.avis)} / 5 </span>
                ...
                <div className="mt-6  justify-start flex-wrap">
                  {hebergement.avis.map((review, index) => (
