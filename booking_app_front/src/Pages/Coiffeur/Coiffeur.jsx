...
import { coiffeurs } from '../../assets/data';
...
const Coiffeur = () => {
  ...
  const coiffeur = coiffeurs.find((barber) => barber.uri === uri);
  const[activeImage,setActiveImage]=useState(coiffeur.images[0]);
  ...
  if (!coiffeur) {
    return <div>Coiffeur not found</div>;
  }
  ...
  return (
    <div className="w-9/12 max-sm:w-full mx-auto bg-transparent shadow-lg  overflow-hidden padding  text-night">
        ...
        <div className="flex-1 px-4 max-sm:px-6 max-lg:px-8">
            <h2 className="headline first-letter:text-customBlue  ">{coiffeur.nom}</h2>
            <hr className='w-1/2' />
            <p className="italic mb-4 text-lg font-semibold py-10 w-11/12 font-sansBody">{coiffeur.slogan}</p>
            <div className='flex flex-wrap w-full my-4 max-md:my-2'> {coiffeur.images.map((image)=>(
        <img 
          ...
          style={{backgroundImage: `url(${activeImage})`,height:"80vh"}}>
            ...
            <div className="my-5 text-lg max-md:text-base ">
                ...
                <div className=' padding w-full text-night font-sansBody font-semibold'>
                 <strong className='text-lg font-bold  xl:headline'>Description :</strong> <p className=" px-6  text-left  "> {coiffeur.description}</p>
                ...
                {/* <p className=" p-5 px-10 border-2 border-customBlue mx-4"><strong className='text-night text-lg'>Adresse:</strong> {coiffeur.adresse}</p> */}
                {/* { coiffeur.téléphone ?
                <p className=" p-5 px-10 border-2 border-customBlue mx-4"><strong className='text-night text-lg'>Téléphone:</strong> {coiffeur.téléphone}</p>
                :""} */}
                {coiffeur.prix ? 
                <p className=" p-5 px-10 border-2 border-customBlue mx-4"><strong className='text-night text-lg'>Prix:</strong> {coiffeur.prix}</p> 
                :""
                }
                {coiffeur.horaires ? 
                <p className=" p-5 px-10 border-2 border-customBlue mx-4"><strong className='text-night text-lg'>Horaires:</strong> {coiffeur.horaires}</p>
                :""}
                {coiffeur.informations_importants?
                coiffeur.informations_importants.notes? 
                 <p className='p-5 px-10 border-2 border-customBlue mx-4 '><strong className='text-night text-lg'>Notation  :</strong> {coiffeur.informations_importants.notes}</p>
                :"":""}

                {/* {coiffeur.siteweb ?
                <p className='capitalize p-5 px-10 border-2 border-customBlue font-bold leading-10 mx-4' >pour visiter le site Web de ce coiffeur Cliquer sur :<a href={coiffeur.siteweb} target="_blank" rel="noopener noreferrer" className="font-bold text-white bg-Lion p-2  ">site web <FontAwesomeIcon icon={faLink} /> </a></p>
                :""} */}
                ...
                <div className='bg-white w-1/5 max-md:w-6/12 max- h-3 rounded-xl mx-4 '>
                    ...
                    style={{width:`${averageRating(coiffeur.avis)*100/5}%`}}>
                    </div>
                  </div>
                  <span className="text-lg font-semibold">  { averageRating(coiffeur.avis)} / 5 </span>
                ...
                <div className="mt-6  justify-start flex-wrap">
                  {coiffeur.avis.map((review, index) => (
