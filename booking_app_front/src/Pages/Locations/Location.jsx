...
const Location = () => {
   ...
   <div className="flex-1 px-4 max-sm:px-6 max-lg:px-8">
       <h2 className="headline first-letter:text-customBlue  ">{location.nom}</h2>
       <hr className='w-1/2' />
       <p className="italic mb-4 text-lg font-semibold py-10 w-11/12 font-sansBody">{location.slogan}</p>
       <div className='flex flex-wrap w-full my-4 max-md:my-2'> {location.images.map((image)=>(
   <img 
   ...
   <div className=" flex-1 w-full h-screen mx-auto bg-cover padding transition-all "
     style={{backgroundImage: `url(${activeImage})`,height:"80vh"}}>
       ...
       <div className="my-5 text-lg max-md:text-base ">
           ...
           {/* {location.siteweb ?
           <p className='capitalize p-5 px-10 border-2 border-customBlue font-bold leading-10 mx-4' >pour visiter le site Web de ce location Cliquer sur :<a href={location.siteweb} target="_blank" rel="noopener noreferrer" className="font-bold text-white bg-Lion p-2  ">site web <FontAwesomeIcon icon={faLink} /> </a></p>
           :""} */}
           ...
           <div className='bg-white w-1/5 max-md:w-6/12 max- h-3 rounded-xl mx-4 '>
               ...
               style={{width:`${averageRating(location.avis)*100/5}%`}}>
               </div>
             </div>
             <span className="text-lg font-semibold">  { averageRating(location.avis)} / 5 </span>
